...
class SceneVisualizer:
    ...
    def __init__(
        ...
        self.scene = scene
        self.states, self.group_states, self.entry_states = self.scene.get_states()
        self.cmap = cmap
    ...
    def plot(self):
        ...
        self.plot_obstacles()
        start = 0
        for t in len(self.entry_states):
            current_entry_num = self.entry_states[t]
            # current_state = self.states[t]
            current_entry_human = [x for x in range(start, start+current_entry_num)]
            for ped in current_entry_human:
                x = self.states[t:, ped, 0]
                y = self.states[t:, ped, 1]
    ...
    def plot_human(self, step=-1):
        ...
        """
        states, _, entry_states = self.scene.get_states()
        current_state = states[step]
        current_entry_num = entry_states[step]
        new_entry_human_state = current_state[len(current_state)-current_entry_num:]
        # radius = 0.2 + np.linalg.norm(current_state[:, 2:4], axis=-1) / 2.0 * 0.3
        radius = [0.2] * current_state.shape[0]
        if len(new_entry_human_state) != 0:
            self.human_actors.append([
                Circle(pos, radius=r) for pos, r in zip(new_entry_human_state[:, :2], radius)
            ])
